...
export class ClineProvider implements vscode.WebviewViewProvider {
	public static readonly sideBarId = "bao-cline.SidebarProvider" // used in package.json as the view's id. This value cannot be changed due to how vscode caches views based on their id, and updating the id would break existing instances of the extension.
			...
			async (message: WebviewMessage) => {
				switch (message.type) {
					...
					case "connectToJs":
						...
						await jsCommunicator.connect()
						this.outputChannel.appendLine("Connected to JS tools")
						break
					case "sendMessageToJs":
						const jsCommunicatorSend = new JsCommunicator()
						await jsCommunicatorSend.sendMessage(message.text!)
						this.outputChannel.appendLine(`Message sent to JS tools: ${message.text}`)
						break
					case "receiveMessageFromJs":
						const jsCommunicatorReceive = new JsCommunicator()
						const receivedJsMessage = await jsCommunicatorReceive.receiveMessage()
						this.outputChannel.appendLine(`Message received from JS tools: ${receivedJsMessage}`)
						await this.postMessageToWebview({ type: "jsMessage", text: receivedJsMessage })
						break
				}
			},
			null,
